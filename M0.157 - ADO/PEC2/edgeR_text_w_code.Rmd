...
```
# 0.[github](https://github.com/jfrojasc/UOC/tree/master/M0.157%20-%20ADO/PEC2/)
...
boxplot(pseudocounts, las=2, col=(c(rep("lightblue",10),rep("blue",10),rep("navyblue",10))), main="Figura 11. Boxplot de las pseudocuentas")
```
...
ggplot(df_ma, aes(x = A, y = M)) + geom_point(size = 1.5, alpha = 1/5) +
geom_hline(color = "blue3", yintercept = 0) + stat_smooth(se = FALSE, method = "loess", color = "red3") + ggtitle("Figura 12. MA plot de NIT vs SFI")
```
...
ggplot(df_ma, aes(x = A, y = M)) + geom_point(size = 1.5, alpha = 1/5) +
geom_hline(color = "blue3", yintercept = 0) + stat_smooth(se = FALSE, method = "loess", color = "red3") + ggtitle("Figura 13. MA plot de SFI vs ELI") 
```
...
ggplot(df_ma, aes(x = A, y = M)) + geom_point(size = 1.5, alpha = 1/5) +
geom_hline(color = "blue3", yintercept = 0) + stat_smooth(se = FALSE, method = "loess", color = "red3") + ggtitle("Figura 14. MA plot de NIT vs ELI")
```

```{R fig.height=20, fig.width=20}
plotMDS(pseudocounts, pch=c(1:6,15,17,18,19), col=c(rep("red",10), rep("blue",10), rep("green",10)), cex=2, main="Figura 15. Gráfico de escalamiento multidimensional de distancias entre perfiles de expresión génica", xlim=c(-5,5), ylim=c(-2,2))
...
No hubo diferencias estadísticamente significativas entre NIT y SFI.<Br />
Hubo pocas diferencias estadísticamente significativas entre NIT y ELI.<Br />
El contraste de SFI vs ELI tuvo algunos datos problemáticos.<Br />

### Expresión diferencial sobre un umbral específico (2x)

Es importante notar que los genes no se ordenan específicamente por pvalor, sino de acuerdo al dendrograma que los agrupa. 
```{R echo=FALSE}
...
rownames(logCPM) <- res_ann_NITvsSFI$SYMBOL
...
cim(t(logCPM), color = sort(hmcol, decreasing=T), symkey = FALSE, margins = c(7, 7))
```
...
rownames(logCPM) <- res_ann_SFIvsELI$SYMBOL
colnames(logCPM) <- as.character(dg_list_2$samples$short_name)
...
cim(t(logCPM), color = sort(hmcol, decreasing=T), symkey = FALSE, margins = c(7, 7))
```
En los heatmaps se observa la misma tendencia: NIT y SFI son muy similares, y hay 2 datos de SFI que se parecen mucho a ELI. 

# Diagnóstico gráfico
```{R}
hist(res_ann_NITvsSFI$PValue, col="lightblue")
hist(res_ann_SFIvsELI$PValue, col="lightblue")
hist(res_ann_NITvsELI$PValue, col="lightblue")

```
El análisis de los histogramas de p-valor coincide con lo que indican los smear plots, y los heatmaps: se observa que los grupos NIT y SFI son muy similares, y que SFI vs ELI están con una proporción alta de genes diferencialmente expresados, pero hay una cantidad sustancial que no están diferencialmente expresados. En el caso de NIT vs ELI, se observa una mayor cantidad de genes diferencialmente expresados, indicado por la barra cercana de cero más alta, y las demás más bajas. El histograma de SFI vs ElI parece indicar que debe revisarse si hay algún problema con los datos. 

# 5. Discusión <Br />
# 5.1. Limitaciones:  <Br />
En todos los casos donde se hacen análisis de proximidad de los grupos, se observa que no hay diferencias claras entre los mismos. Sin embargo, algunos genes mostraron patrones de expresión diferencial. <Br />
Se observó un patrón de datos no deseable en el histograma de SFI vs ELI. <Br />
