...
```{r}
which(tabla_0[,5]==max(tabla_1[,5]))
n_pos <- CMs[[which(tabla_0[,5]==max(tabla_1[,5]))]]$t[1]
n_neg <- CMs[[which(tabla_0[,5]==max(tabla_1[,5]))]]$t[4]
n_fp <- CMs[[which(tabla_0[,5]==max(tabla_1[,5]))]]$t[3]
n_fn <- CMs[[which(tabla_0[,5]==max(tabla_1[,5]))]]$t[2]
```

Los porcentajes de las celdas en la tabla indican la proporción de valores, que se dividen en cuatro categorías. La celda superior izquierda indica los resultados **negativos reales**. Estos `r n_pos` de `r num_test` valores son casos en los que la masa era benigna y el algoritmo k-NN la identificaba correctamente como tal. La celda inferior derecha indica los resultados **positivos reales**, donde el clasificador y la etiqueta clínicamente determinada coinciden en que la masa es maligna. Un total de `r n_neg` de `r num_test` predicciones fueron verdaderos positivos.
...
Un total de `r n_fn` de cada `r num_test`, o el `r (CMs[[which(tabla_0[,5]==max(tabla_1[,5]))]]$t[2]/num_test)*100` por ciento de las masas fueron clasificadas incorrectamente por el enfoque k-NN. Si bien la precisión del `r 100-(r CMs[[which(tabla_0[,5]==max(tabla_1[,5]))]]$t[2]/num_test)` por ciento parece impresionante para algunas líneas de código R, se puede intentar otra iteración del modelo para ver si podemos mejorar el rendimiento y reducir la cantidad de valores que se han clasificado incorrectamente, particularmente porque los errores falsos negativos, son peligrosos.
